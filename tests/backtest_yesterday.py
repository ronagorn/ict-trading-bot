...
def run_yesterday_backtest():
    print("🚀 Starting Yesterday (Last 24-48 Hours) Backtest on Real XM Data...")
    client = MT5Client()
    ...
    for symbol in symbols:
        ...
        # ตรวจสอบแท่งย้อนหลัง 2 วันล่าสุด
        for i in range(len(df) - 192, len(df) - 15):
    ...
    report = f"📅 <b>REAL XM BACKTEST REPORT (เมื่อวาน / 24-48 ชั่วโมงล่าสุด)</b>\n"
    report += f"📊 <b>รวม 10 คู่เงินหลัก XM</b>\n\n"
    
    if all_executed_trades:
        report += "<b>📋 รายการออเดอร์เทรดจริงที่เกิดขึ้นเมื่อวาน:</b>\n"
        ...
        report += "ไม่มีสัญญาณออเดอร์ที่ผ่านฟิลเตอร์ความแม่นยำช่วง 24 ชม. ล่าสุด\n"
        
    report += "\n----------------------------------------\n"
    report += f"🏆 <b>สรุปผลเมื่อวาน (ทุน $100):</b>\n"
    report += f"🔹 สัญญาณเทรดรวม: <b>{total_trades} ไม้</b>\n"
    report += f"🔹 ชนะ (TP 1:1.5): <b>{total_wins} ไม้</b> | แพ้ (SL): <b>{total_losses} ไม้</b>\n"
    report += f"🔹 <b>Win Rate เมื่อวาน: {winrate:.2f}%</b>\n"
    report += f"💵 <b>กำไรสุทธิรวมเมื่อวาน (ทุน $100): ${total_net_pnl:.2f} (ROI: {total_net_pnl:.2f}%)</b>\n"
    report += f"💵 <b>กำไรสุทธิรวมเมื่อวาน (ทุน $1,000): ${total_net_pnl*10:.2f} (ROI: {total_net_pnl:.2f}%)</b>\n"
    ...
    if tg.enabled:
        ...
        print("Yesterday Backtest Report sent to Telegram.")
