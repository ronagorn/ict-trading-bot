...
def run_day30_capped():
    print("🚀 Running Thursday 30 July 2026 Backtest WITH Daily 3% Drawdown Limit...")
    client = MT5Client()
    ...
    for symbol in symbols:
        ...
        for i in group.index:
            ...
            if is_stopped: break # บังคับใช้ระบบเซฟตี้ 3% ประจำวัน!
    ...
    report = f"📅 <b>REAL XM BACKTEST (30 กรกฎาคม 2026 - บังคับใช้ระบบเซฟตี้ 3% Daily Cap)</b>\n"
    report += f"📊 <b>รวม 10 คู่เงินหลัก XM</b>\n\n"
    
    for sym, res in symbol_results.items():
        status = "🛑 สั่งหยุดเทรด (-3%)" if res["stopped"] else "🟢 ทำงานปกติ"
        report += f"<b>📌 {sym}</b>: ชนะ: {res['wins']} | แพ้: {res['losses']} | PnL: {res['pnl']:+.1f}% | {status}\n"
        
    report += "\n----------------------------------------\n"
    report += f"🏆 <b>สรุปผลวันวานซืน (30 ก.ค.) แบบบังคับเซฟตี้ 3% (ทุน $100):</b>\n"
    ...
    if tg.enabled:
        ...
        print("Day 30 Capped Backtest Report sent to Telegram.")
