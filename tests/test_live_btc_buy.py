...
def run_test_btc_buy():
    print("🚀 Initiating Test Demo BUY Order Demonstration for BTCUSD# (Crypto 24/7 Market)...")
    tg = TelegramNotifier()
    client = MT5Client(tg)
    ...
    # SL $500 below, TP $750 above (RR 1:1.5)
    sl_price = entry_price - 500.0
    tp_price = entry_price + 750.0
    
    # 2. Calculate Lot Size based on Risk Manager
    lot_size = risk_mgr.calculate_lot_size(acc_info.equity, sym_info, entry_price, sl_price)
    if lot_size <= 0:
        lot_size = sym_info.volume_min # Default to min volume
    ...
    ticket = client.place_order(symbol, "BUY", lot_size, entry_price, sl_price, tp_price, comment="Demo_Test_BTC")
    
    if ticket:
        print(f"✅ SUCCESS! Ticket #{ticket} placed on MT5 Demo!")
        
        # Send Telegram notification
        msg = f"🧪 <b>DEMO TEST ORDER EXECUTED [BTCUSD#]</b>\n\n"
        ...
        print("❌ Order placement failed. Please check MT5 symbol info.")
